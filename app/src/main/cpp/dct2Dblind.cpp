//
// Created by rostislav on 19.10.18.
//
...
//Сalculate coefficient mean, var, skewness, kurtosiss
void calculateCoefficient(double *matrix, int cols, int rows, double *coff){
...
void dct2Dblind(double *image, int cols, int rows, double *coff)
{
    ...
    for(int freq = 0; freq < 4; freq++)
    {
        ...
        *(coff + (freq*4)) = mean;
        *(coff + (freq*4)+1) = variance;
        *(coff + (freq*4)+2) = skewness;
        *(coff + (freq*4)+3) = kurtosis;
    }
}
